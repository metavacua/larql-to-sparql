...
fn main() {
    ...
    #[cfg(feature = "metal")]
    {
        ...
        let build_layers = |count: usize| -> Vec<Layer> {
            (0..count).map(|l| {
                ...
                Layer {
                    ...
                    g: quantize_q4_0(&(0..inter*hidden).map(|i| ((i+l*5000) as f32*0.0001).cos()).collect::<Vec<_>>()),
                    u: quantize_q4_0(&(0..inter*hidden).map(|i| ((i+l*6000) as f32*0.0002).sin()).collect::<Vec<_>>()),
                    d: quantize_q4_0(&(0..hidden*inter).map(|i| ((i+l*7000) as f32*0.0003).cos()).collect::<Vec<_>>()),
        ...
        let q4k_21: Vec<larql_compute::FullPipelineLayer> = data_21.iter().map(|l| larql_compute::FullPipelineLayer {
            ...
            gate: larql_compute::QuantWeight { data: &l.g, scales: None, format: larql_compute::QuantFormat::Q4_0 },
