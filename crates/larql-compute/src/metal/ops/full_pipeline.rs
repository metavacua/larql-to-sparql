...
fn dispatch_ffn_matvec(
    ...
    quant_matvec::encode(
        enc, format, w_buf, f32_in, f32_in_off, q8_in, q8_in_off, q8s_in, q8s_in_off, out_buf,
        out_off, &pipes, num_rows, hidden,
    );
}
...
pub fn dispatch_full_pipeline(
    ...
    let _wo_scale_bufs: Vec<_> = layers
        .iter()
    ...
    if let Some(ref dir) = dump_path {
        ...
        if !ptr.is_null() {
            ...
            let bytes: Vec<u8> = s.iter().flat_map(|v: &f32| v.to_le_bytes()).collect();
    ...
    for l in 0..num_layers {
        ...
        let fused_qkv_pipe = q4kf_qkv_proj_pipeline
            .or(q4k_qkv_proj_pipeline)
            .filter(|_| {
                all_same_format
                    && matches!(
                        layers[l].wq.format,
        ...
        if dump_path.is_some() && l == 0 {
            ...
            if !ptr.is_null() {
                ...
                let _ = std::fs::write(
                    format!(
                        "{}/metal_L0_q_out_after_qk_norm.f32",
                        dump_path.as_ref().unwrap()
                    ),
                    &bytes,
                );
            }
    ...
    if let Some(ref mut kv) = kv_cache {
        for l in 0..num_layers {
            ...
            while kv.layers.len() <= l {
                kv.layers
                    .push(super::kv_cache::LayerKVCache::new(bufs, 4096, lnkv, lhd));
            }
