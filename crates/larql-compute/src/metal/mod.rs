...
impl MetalBackend {
    ...
    pub fn new() -> Option<Self> {
        ...
        let f32_gemv_pipeline = device
            .new_compute_pipeline_state_with_function(&f32_gemv_fn)
            .ok()?;
        // f16 counterpart — half the memory, same shader topology.
        let f16_gemv_fn = library.get_function("f16_gemv", None).ok()?;
        let f16_gemv_pipeline = device
            .new_compute_pipeline_state_with_function(&f16_gemv_fn)
            .ok()?;
        ...
        let q4k_q6k_qkv_proj_pipeline = device
            .new_compute_pipeline_state_with_function(&q4k_q6k_qkv_fn)
            .ok()?;
        ...
        let qk_norm_pipeline = device
            .new_compute_pipeline_state_with_function(&qk_norm_fn)
            .ok()?;
