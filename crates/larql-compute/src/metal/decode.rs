...
impl MetalBackend {
    ...
    ///
    /// **Merged encoder architecture** (3 encoders per layer):
    ///   - Encoder A: input norm → QKV projection → RoPE → V-norm
    ///   - Encoder B: KV cache append → KV attend (needs barrier for cache write)
    ///   - Encoder C: O projection → residual+norm+Q8 → FFN → post-FFN residual → layer scalar
    ///
    /// This reduces dispatch overhead from ~10 encoders/layer to 3, saving ~0.15ms × 7 × 34
    /// = ~36ms of GPU idle time for a 34-layer model.
    ...
    pub fn decode_token(
        ...
        for l in 0..num_layers {
            ...
            // ═══════════════════════════════════════════════════════════
            // ENCODER A: Input norm → QKV projection → RoPE → V-norm
            // All pre-attention work in one encoder.
            // ═══════════════════════════════════════════════════════════
            ...
            if uses_q4k {
                ...
                if layer.norm_type == crate::NormType::LayerNorm {
                    ...
                    if let Some(bias) = layer.input_norm_bias {
                        ...
                        enc_a.set_compute_pipeline_state(&self.layer_norm_pipeline);
                        enc_a.set_buffer(0, Some(&h_buf), 0);
                        enc_a.set_buffer(1, Some(&input_norm_bufs[l]), 0);
                        enc_a.set_buffer(2, Some(&bias_buf), 0);
                        enc_a.set_buffer(3, Some(&norm_f32_buf), 0);
                        enc_a.set_bytes(4, 4, &len_val as *const u32 as *const std::ffi::c_void);
                        ...
                        enc_a.set_compute_pipeline_state(&self.layer_norm_no_bias_pipeline);
                        enc_a.set_buffer(0, Some(&h_buf), 0);
                        enc_a.set_buffer(1, Some(&input_norm_bufs[l]), 0);
                        enc_a.set_buffer(2, Some(&norm_f32_buf), 0);
                        enc_a.set_bytes(3, 4, &len_val as *const u32 as *const std::ffi::c_void);
                        enc_a.set_bytes(4, 4, &eps as *const f32 as *const std::ffi::c_void);
                        enc_a.set_bytes(5, 4, &norm_offset as *const f32 as *const std::ffi::c_void);
                    }
                ...
                // Dispatch 2: Q4_K QKV
                enc_a.set_compute_pipeline_state(&self.q4kf_qkv_proj_pipeline);
                enc_a.set_buffer(0, Some(&wq_bufs[l]), 0);
                enc_a.set_buffer(1, Some(&wk_bufs[l]), 0);
                enc_a.set_buffer(2, Some(&wv_bufs[l]), 0);
                enc_a.set_buffer(3, Some(&norm_f32_buf), 0);
                ...
                enc_a.set_buffer(4, Some(&wq_scale_bufs[l]), 0);
                enc_a.set_buffer(5, Some(&wk_scale_bufs[l]), 0);
                enc_a.set_buffer(6, Some(&wv_scale_bufs[l]), 0);
                ...
                enc_a.set_bytes(11, 4, &q_rows as *const u32 as *const std::ffi::c_void);
                enc_a.set_bytes(12, 4, &k_rows as *const u32 as *const std::ffi::c_void);
                enc_a.set_bytes(13, 4, &v_rows as *const u32 as *const std::ffi::c_void);
                enc_a.set_bytes(14, 4, &k_val as *const u32 as *const std::ffi::c_void);
                enc_a.dispatch_thread_groups(
            ...
            // ── Step 4: O projection (own encoder — output feeds into encoder C) ──
            ...
            {
                let enc_o = cmd.new_compute_command_encoder();
                if uses_q4k {
                    ...
                    enc_o.set_compute_pipeline_state(&self.q4kf_proj_pipeline);
                    enc_o.set_buffer(0, Some(&wo_bufs[l]), 0);
                    enc_o.set_buffer(1, Some(&attn_out), 0);
                    enc_o.set_buffer(2, Some(&o_out_buf), 0);
                    enc_o.set_bytes(3, 4, &o_rows as *const u32 as *const std::ffi::c_void);
                    enc_o.set_bytes(4, 4, &o_k as *const u32 as *const std::ffi::c_void);
                    enc_o.dispatch_thread_groups(
                        MTLSize::new(num_tgs, 1, 1),
                    ...
                    enc_o.set_compute_pipeline_state(&self.q8_quant_pipeline);
                    enc_o.set_buffer(0, Some(&attn_out), 0);
                    enc_o.set_buffer(1, Some(&o_q8), 0);
                    enc_o.set_buffer(2, Some(&o_q8s), 0);
                    enc_o.set_bytes(3, 4, &dim_val as *const u32 as *const std::ffi::c_void);
                    enc_o.dispatch_threads(MTLSize::new(blocks as u64, 1, 1), MTLSize::new(256.min(blocks as u64), 1, 1));

                    let o_rows = hidden as u32;
                    let o_k = layer_q_dim as u32;
                    enc_o.set_compute_pipeline_state(&self.q8_matvec_pipeline);
                    enc_o.set_buffer(0, Some(&wo_bufs[l]), 0);
                    enc_o.set_buffer(1, Some(&o_q8), 0);
                    enc_o.set_buffer(2, Some(&wo_scale_bufs[l]), 0);
                    enc_o.set_buffer(3, Some(&o_q8s), 0);
                    enc_o.set_buffer(4, Some(&o_out_buf), 0);
                    enc_o.set_bytes(5, 4, &o_rows as *const u32 as *const std::ffi::c_void);
                    enc_o.set_bytes(6, 4, &o_k as *const u32 as *const std::ffi::c_void);
                    enc_o.dispatch_thread_groups(
            ...
            // ═══════════════════════════════════════════════════════════
            // ENCODER C (actual): residual+norm+Q8 → FFN → post-FFN → scalar
            // Merged: steps 5 + 6 + 7 + 8 in one encoder.
            // ═══════════════════════════════════════════════════════════
            ...
            if has_post_norms {
                ...
                enc_c.set_compute_pipeline_state(&self.residual_norm_q8_pipeline);
                enc_c.set_buffer(0, Some(&h_buf), 0);
                enc_c.set_buffer(1, Some(&normed_o), 0);
                enc_c.set_buffer(2, Some(&pre_ffn_buf), 0);
                enc_c.set_buffer(3, Some(&ffn_q8), 0);
                enc_c.set_buffer(4, Some(&ffn_q8s), 0);
                enc_c.set_buffer(5, Some(&h_post_attn), 0);
                enc_c.set_bytes(6, 4, &hidden_val as *const u32 as *const std::ffi::c_void);
                enc_c.set_bytes(7, 4, &eps as *const f32 as *const std::ffi::c_void);
                enc_c.set_bytes(8, 4, &norm_offset as *const f32 as *const std::ffi::c_void);
                enc_c.dispatch_threads(MTLSize::new(hidden as u64, 1, 1), MTLSize::new(256.min(hidden as u64), 1, 1));
            } else {
                enc_c.set_compute_pipeline_state(&self.residual_norm_q8_pipeline);
                enc_c.set_buffer(0, Some(&h_buf), 0);
                enc_c.set_buffer(1, Some(&o_out_buf), 0);
                enc_c.set_buffer(2, Some(&post_attn_norm_bufs[l]), 0);
                enc_c.set_buffer(3, Some(&ffn_q8), 0);
                enc_c.set_buffer(4, Some(&ffn_q8s), 0);
                enc_c.set_buffer(5, Some(&h_post_attn), 0);
                enc_c.set_bytes(6, 4, &hidden_val as *const u32 as *const std::ffi::c_void);
            ...
            // ── Step 6: FFN ──
            {
                ...
                if layer.is_gated() {
                    ...
                    enc_c.set_compute_pipeline_state(&self.q4.matvec);
                    enc_c.set_buffer(0, Some(&gate_bufs[l]), 0);
                    enc_c.set_buffer(1, Some(&ffn_q8), 0);
                    enc_c.set_buffer(2, Some(&ffn_q8s), 0);
                    enc_c.set_buffer(3, Some(&gate_out), 0);
                    enc_c.set_bytes(4, 4, &inter_val as *const u32 as *const std::ffi::c_void);
                    enc_c.set_bytes(5, 4, &hidden_val as *const u32 as *const std::ffi::c_void);
                    enc_c.dispatch_thread_groups(MTLSize::new(n_tgs_ffn, 1, 1), MTLSize::new(q4mv::THREADS_PER_TG, 1, 1));
                    enc_c.set_buffer(0, Some(&up_bufs[l]), 0);
                    enc_c.set_buffer(3, Some(&up_out), 0);
                    enc_c.dispatch_thread_groups(MTLSize::new(n_tgs_ffn, 1, 1), MTLSize::new(q4mv::THREADS_PER_TG, 1, 1));
                    let geglu = match layer.activation {
                        crate::Activation::GeluTanh => &self.geglu_gelu_tanh_pipeline,
                        _ => &self.geglu_pipeline,
                    };
                    enc_c.set_compute_pipeline_state(geglu);
                    enc_c.set_buffer(0, Some(&gate_out), 0);
                    enc_c.set_buffer(1, Some(&up_out), 0);
                    enc_c.set_buffer(2, Some(&act_buf), 0);
                    enc_c.set_bytes(3, 4, &inter_val as *const u32 as *const std::ffi::c_void);
                    enc_c.dispatch_threads(MTLSize::new(inter as u64, 1, 1), MTLSize::new(256, 1, 1));
                } else {
                    enc_c.set_compute_pipeline_state(&self.q4.matvec);
                    enc_c.set_buffer(0, Some(&up_bufs[l]), 0);
                    enc_c.set_buffer(1, Some(&ffn_q8), 0);
                    ...
                    enc_c.set_compute_pipeline_state(activation_pipeline);
                    enc_c.set_buffer(0, Some(&up_out), 0);
                    enc_c.set_buffer(1, Some(&act_buf), 0);
                    enc_c.set_bytes(2, 4, &inter_val as *const u32 as *const std::ffi::c_void);
                    enc_c.dispatch_threads(MTLSize::new(inter as u64, 1, 1), MTLSize::new(256, 1, 1));
                }

                enc_c.set_compute_pipeline_state(&self.q4.f32_matvec);
                enc_c.set_buffer(0, Some(&down_bufs[l]), 0);
                enc_c.set_buffer(1, Some(&act_buf), 0);
                enc_c.set_buffer(2, Some(&down_out), 0);
                enc_c.set_bytes(3, 4, &hidden_val as *const u32 as *const std::ffi::c_void);
                enc_c.set_bytes(4, 4, &inter_val as *const u32 as *const std::ffi::c_void);
                enc_c.dispatch_threads(MTLSize::new(hidden as u64, 1, 1), MTLSize::new(256, 1, 1));
            }
            ...
            if has_post_norms {
                ...
                let len_val = hidden as u32;
                enc_c.set_compute_pipeline_state(&self.residual_add_pipeline);
                enc_c.set_buffer(0, Some(&h_post_attn), 0);
                enc_c.set_buffer(1, Some(&down_out), 0);
                enc_c.set_buffer(2, Some(&new_h), 0);
                enc_c.set_bytes(3, 4, &len_val as *const u32 as *const std::ffi::c_void);
                enc_c.dispatch_threads(MTLSize::new(hidden as u64, 1, 1), MTLSize::new(256.min(hidden as u64), 1, 1));
            }
            ...
            if layer.layer_scalar != 0.0 {
                ...
                enc_c.set_compute_pipeline_state(&self.scale_vector_pipeline);
                enc_c.set_buffer(0, Some(&new_h), 0);
                enc_c.set_buffer(1, Some(&scaled), 0);
                enc_c.set_bytes(2, 4, &hidden_val as *const u32 as *const std::ffi::c_void);
                enc_c.set_bytes(3, 4, &scalar_val as *const f32 as *const std::ffi::c_void);
                enc_c.dispatch_threads(MTLSize::new(hidden as u64, 1, 1), MTLSize::new(256.min(hidden as u64), 1, 1));
                enc_c.end_encoding();
                h_buf = scaled;
            } else {
