...
fn predict_kquant_decode_step_direct_runs_with_q4k_fixture() {
    ...
    let result = predict_kquant_decode_step_direct(&weights, 4u32, &idx, &backend, &mut cache, 3);
    match result {
...
fn fused_decode_step_with_state_masked_drives_through_mock_backend() {
    ...
    for mask in [
        ...
        let result =
            fused_decode_step_with_state_masked(&weights, &idx, 0u32, &backend, &mut dump, mask);
        assert!(
            result.is_some(),
