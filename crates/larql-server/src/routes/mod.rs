...
pub fn single_model_router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(
            "/v1/expert/{layer}/{expert_id}",
            post(expert::handle_expert),
        )
...
pub fn multi_model_router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(
            "/v1/{model_id}/embed/{token_id}",
            get(embed::handle_embed_single_multi),
        )
        .route("/v1/{model_id}/logits", post(embed::handle_logits_multi))
        .route(
            "/v1/{model_id}/token/encode",
            get(embed::handle_token_encode_multi),
        )
        .route(
            "/v1/{model_id}/token/decode",
            get(embed::handle_token_decode_multi),
        )
        .with_state(state)
}
