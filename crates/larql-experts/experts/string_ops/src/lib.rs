...
expert_exports!(
    ...
    tier = 1,
    description =
        "String operations: reverse, palindrome, anagram, count, caesar cipher, rot13, case",
    version = "0.2.0",
    ops = [
        ("reverse", ["s"]),
        ("is_palindrome", ["s"]),
...
fn dispatch(op: &str, args: &Value) -> Option<Value> {
    match op {
        ...
        "starts_with" => Some(json!(
            arg_str(args, "s")?.starts_with(arg_str(args, "prefix")?)
        )),
        "ends_with" => Some(json!(
            arg_str(args, "s")?.ends_with(arg_str(args, "suffix")?)
        )),
        _ => None,
    }
}
