...
//!
//! Run: cargo run -p larql-demos --example demo_loading -- /path/to/model
...
fn main() {
    ...
    if args.len() < 2 {
        ...
        println!();
        println!("Examples:");
        println!("  cargo run -p larql-demos --example demo_loading -- /path/to/gemma-3-4b");
        println!("  cargo run -p larql-demos --example demo_loading -- model.gguf");
        return;
    }
