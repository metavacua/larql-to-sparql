...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The default above is a convenience, not a guarantee — it names one
    // artifact out of many a checkout might hold. Fail by name rather than
    // letting `open_inference_vindex` surface a bare `NotFound` that says
    // neither which path was tried nor how to change it.
    if !vindex_path.exists() {
        return Err(format!(
            "vindex not found: {}\npass --vindex <path> to point at one you have",
            vindex_path.display()
        )
        .into());
    }
