...
// card::render is portable on every target now (DESIGN A, see
// card/body/mod.rs): only its Recipe section needs serde_yaml, and
// that's narrowed to render_body_with_recipe inside card::render
// itself, native-only. Recipe::from_yaml below is still native-only in
