...
Usage:
  ...
  python3 tools/census.py --gpu           # add GPU target compile probes (slow; requires optional toolchains)
"""
...
from wasm_gate_common import WASM_DIR, ROOT, MANIFEST, kernel_crates, cargo_check, cargo_check_target
...
def cmd_inventory(crates: list[str]) -> None:
    """Emit .github/wasm-inventory.md: per-crate compile status (authoritative,
    via cargo) + the SET A gated surface. Redirect stdout to regenerate the doc."""
    print("# WASM boundary inventory")
    print()
    print("Generated by `crates/larql-wasm/tools/census.py --inventory` — do not hand-edit;")
    print("regenerate on demand. The census is the source of truth for the wasm32v1-none")
    print("compile boundary.")
    print()
    print("## Legend")
    print()
    ...
    print("| ⏸ | deferred — test-run columns are rewritten in the interface refactor |")
    print()
    print("**wasm32v1-none** = `cargo check --target wasm32v1-none --lib`; **native** = "
          "`cargo check --lib`. The `wasmi`/`node`/`firefox` test-RUN columns (`cargo test`) "
          "are intentionally deferred to the interface phase and tracked in CI, not here.")
    print()
    print("## Compile status")
    print()
    print("| crate | wasm32v1-none | native | wasmi | node | firefox |")
    print("|-------|:---:|:---:|:---:|:---:|:---:|")
    for base in crates:
        pkg = f"{base}-wasm32v1-none"
        print(f"  checking {pkg} ...", file=sys.stderr)
        w = "✅" if cargo_check(pkg, wasm=True)[0] else "❌"
        n = "✅" if cargo_check(pkg, wasm=False)[0] else "❌"
        print(f"| {base} | {w} | {n} | ⏸ | ⏸ | ⏸ |")
    print()
    wasm_deps = crate_deps(crates, wasm=True)
    gated_crates = sorted(crate_deps(crates, wasm=False) - wasm_deps)
    t = scan_stdlib(crates)
    print("## Gated surface (SET A) — outside the portable kernel")
    print()
    print(f"**Native-only external crates ({len(gated_crates)}):** "
          + ", ".join(f"`{c}`" for c in gated_crates))
    print()
    print("**Gated stdlib modules:** "
          + ", ".join(f"`{k}`×{c}" for k, c in t.gated.most_common()))
    print()
    print("> Note: `std::collections` here is the native half of the `hashbrown` prelude "
          "split (already portable on wasm via hashbrown), not a conversion target.")
    print()
    print(f"**Portable surface (SET B):** {len(wasm_deps)} external crates "
          f"(`{'`, `'.join(sorted(wasm_deps))}`) + `core::`/`alloc::`.")
    print()
    if t.anomalies:
        print(f"> ⚠️ {len(t.anomalies)} active `use std::` leak(s) — boundary NOT clean.")
    else:
        print("> Invariant holds: no active `use std::` in wasm-reachable code.")


def cmd_gpu(crates: list[str]) -> None:
    """Print a GPU portability probe table for the kernel crates.

    Probes three non-standard targets (wasm32-wasip1, spirv-unknown-vulkan1.2,
    nvptx64-nvidia-cuda). Result is True/False/None (None = toolchain absent).

    None cells are rendered as ⏸ (not yet provable) rather than ❌ (proven to
    fail). GPU targets require special toolchains not present on standard CI
    runners; use .github/workflows/gpu-portability.yml for those probes.
    """
    GPU_TARGETS = [
        ("wasm32-wasip1",          "wasi"),
        ("spirv-unknown-vulkan1.2", "spirv"),
        ("nvptx64-nvidia-cuda",    "nvptx64"),
    ]
    header = "| crate |" + "".join(f" {label} |" for _, label in GPU_TARGETS)
    sep    = "|-------|" + "".join(":------:|" for _ in GPU_TARGETS)
    print("## GPU portability probes")
    print()
    print("> Generated by `census.py --gpu`. Requires optional toolchains;")
    print("> ⏸ = target toolchain absent (not a failure). See")
    print("> `.github/workflows/gpu-portability.yml` for CI probes.")
    print()
    print(header)
    print(sep)
    for base in crates:
        pkg = f"{base}-wasm32v1-none"
        row = f"| {base} |"
        for target, _label in GPU_TARGETS:
            print(f"  probing {pkg} --target {target} ...", file=sys.stderr)
            result, _out = cargo_check_target(pkg, target)
            if result is None:
                cell = "⏸"
            elif result:
                cell = "✅"
