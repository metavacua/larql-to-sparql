...
impl<'a> BuildContext<'a> {
    ...
    fn run_clustering(&mut self) -> Result<(), VindexError> {
        run_clustering_pipeline(
            ClusterData {
                directions: core::mem::take(&mut self.cluster_directions),
                features: core::mem::take(&mut self.cluster_features),
                top_tokens: core::mem::take(&mut self.cluster_top_tokens),
                input_tokens: core::mem::take(&mut self.cluster_input_tokens),
                output_tokens: core::mem::take(&mut self.cluster_output_tokens),
            },
            self.hidden_size,
            self.weights,
            self.tokenizer,
