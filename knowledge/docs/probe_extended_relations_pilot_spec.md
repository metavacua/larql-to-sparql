...
`knowledge/data/wordnet_extended_relations.json` — not yet generated; produced by running `fetch_wordnet_extended_relations.py` (draft already exists at that path). Contains 6 relations × ~3000 pairs each. Subject filter: alpha + length ≥ 3. No multi-piece filter (1c tests relation coverage, not surface form). No canonical-skip (1c relations don't appear in canonical).

`knowledge/data/wordnet_extended_relations_provenance.json` — also not yet generated by that same run; records per-relation counts and the adjective/verb split.
