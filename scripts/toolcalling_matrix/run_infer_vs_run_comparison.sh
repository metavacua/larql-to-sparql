...
run_out=$("$larql_bin" run "$vindex_dir" "$PROMPT" -n 5 2>&1)
run_ec=$?
echo "=== larql run (cached KV-decode) ==="
echo "$run_out"
...
if [ "$infer_ec" -ne 0 ] && echo "$infer_out" | grep -qi "requires model weights"; then
  ...
  if [ "$run_first_word" = "$infer_first_tok" ]; then
    ...
    detail="run's first generated token ('$run_first_word') matches INFER's top-1 prediction ('$infer_first_tok') -- no evidence of a cached-decode-path bug independent of chat-template/decoding-strategy hypotheses for this prompt"
  else
    outcome="divergence_found"
    detail="run's first generated token ('$run_first_word') does NOT match INFER's top-1 prediction ('$infer_first_tok') -- possible cached-decode-path bug independent of Q1/Q2 (chat-template/decoding-strategy); escalate to Root-Cause Analysis before trusting either uncommitted fix's own test results"
  fi
fi
...
python3 "$script_dir/write_leg_result.py" \
  ...
  --extra "{\"run_exit\": $run_ec, \"infer_exit\": $infer_ec}" --out "$out_json"
