...
**R8 — Disagreement-point oracle.** Validate an implementation at an input where the competing hypotheses produce **different** values. An oracle exercised only at an equivalence point cannot distinguish the intended implementation from a plausible wrong one, and will pass while the code is broken. Instances: slot 0 of an expert bank, where every offset convention yields zero (the K2 tournament read slots 1–15 from the wrong table and a slot-0 oracle passed); CN-10's rank-1 first answer token, which made "top-1 from L7" appear on every prompt; shared-vs-per-slot input, which only diverges from slot 1; a square `A_log` fixture, where per-head and per-channel readings coincide on the diagonal. The rule generalises: pick the probe point *after* enumerating the plausible wrong implementations, not before. Two further instances, both on the *same* control in one sitting (packed-MoE `GateUpLayout`, 2026-08-10): the borrowed Gemma-4 fixture fills `gate_up` from `i & 0xff`, periodic across the row block, so contiguous-halves and interleaved read identical numbers and the control passed at `max|delta| == 0`; the replacement stamped a value per `(expert, row)` that was **additive** and constant along the row, so summed over `hidden` every row's dot product came out nearly equal and permuting rows moved the result 0.03% — under the test's own bar, and blind to transpose and column-stride besides. The constructive form: make the fixture **multiplicative across every axis** (`f(expert, row, column, branch)`), give the axis under permutation the dominant amplitude, and distrust all-zeros, all-ones, repeated rows, identical experts and periodic ramps in any layout test. Note this is a *fixture* failure distinct from extent (`feedback_fixture_too_small_is_an_absent_test`): the fixture was large enough and still could not separate the hypotheses.
...
**R15 — Name the gauge freedom before comparing two objects elementwise.** Before any cosine, mutual information, residual energy or delta between two weight tensors, ask which axes are canonical and which are a symmetry of the architecture. A comparison along a free axis tests one arbitrary alignment out of many, and a null result from it constrains almost nothing.

The concrete case (DEC-8.13, 2026-08-10): a routed expert computes `w2 · act(w1 x, w3 x)`, so permuting the rows of `w1`/`w3` and the matching columns of `w2` leaves the expert's function **exactly** unchanged. The intermediate index is therefore arbitrary per expert and the checkpoint carries no canonical ordering — for K3's `w1 [3072, 1792]` the rows are free and only the columns (shared input channels) are aligned. The first cross-expert census aligned on both and was sampling one pairing out of `3072!`. Synthetic fixtures could not surface it, because they generate banks in a fixed row order, which is precisely the assumption under test.

Two constructive halves, and both are needed. **Pool over the free axis** — a per-column profile is invariant to the row permutation, so its result generalises and a decoder could use it without any row correspondence. **Run a positive control on the real bytes, not just on fixtures** — `H(t|t) = 0` with agreement 1.0 proves the instrument registers dependence in the exact arrays being scored; without it, "we measured nothing" and "the instrument is blind" are the same output. This is the constructive form of `feedback_controls_before_parity`, and it generalises to every permutation-symmetric unit: attention heads, FFN hidden units, and the expert index itself (hence leave-one-out, order-independent prototypes).

**R16 — Mechanism nulling. To find *why* two engines disagree, run them where the suspect mechanism is structurally inert.** Layer bisection answers *where* divergence appears; nulling answers *why*. Pick a configuration in which a suspected term provably cannot affect the output, and ask whether the disagreement survives. If it vanishes there, the term is implicated; if it persists, the term is exonerated — and either way the answer names a mechanism rather than a line number. R16 is R8's dual: R8 picks a **disagreement point** to validate an implementation, R16 picks an **equivalence point** to localise a defect.

The case (OLMoE, 2026-08-10): `shannon verify` disagreed with HF by 68.336%, and the layer diff put first drift at layer 0 — a location, not a cause. The discriminating observation was in the same dump: **token 0 sat at cos 1.000000000 while the other 272 carried ~61% relative error each.** At position 0 attention softmaxes over a single key, so the weighting is 1.0 whatever Q and K contain; anything touching only q/k is structurally inert there, while V, the output projection, every norm and the whole MoE block still run. One observation exonerated all of them and localised the defect to the q/k path, which was QK-norm applied per-head at a hardcoded epsilon instead of through `qk_norm_scope()` and the architecture's own. Fix confirmed at 0.001%.

Structural controls worth reaching for, each nulling one mechanism: **single-position context** (Q/K weighting, and cache history); **k = num_experts** (top-k selection truncation); **one routed expert** (routing competition); **uniform router logits** (tie-break and normalisation behaviour); **single head or single KV group** (head/group index ambiguity); **unpadded geometry** (padding-layout semantics); **identity or zeroed projections** (an individual FFN branch). Enumerate the suspects first, then choose the control that kills exactly one of them.

### Four conventions this ladder now fixes
...
**Lossless expert transport coding is closed, and four named ideas go with it.** Instrument: `larql k3-ledger cond-census` (DEC-8.13 / ETC-0A, 2026-08-10), conditional entropy on the **untouched** packed MXFP4 nibbles — no subtraction, no requantisation, so the statistic carries no distortion of its own and needs no fidelity label. Layer 49, 16 experts, ~3.7 M scored symbol pairs per arm, 3.3–3.9 MB of range-GET per branch.

| branch | marginal `H(t)` | best `H(t\|context)` | MI |
|---|---|---|---|
| w1 gate | 3.7526 | 3.7526 | 0.0000 |
| w3 up | 3.7521 | 3.7520 | 0.0000 |
| w2 down | 3.7548 | 3.7547 | 0.0000 |

Every arm — leave-one-out bank prototype, out-of-sample-selected parent, random parent, permutation-invariant column profile — sits **on top of its own marginal-preserving shuffled null**. Symbol agreement is 0.086–0.109 against a 1/16 chance floor, so the runnable match/escape code prices at **4.56–4.66 bpw, worse than shipping the raw 4.25** (R11: the entropy floor and the code a kernel can run are reported as a pair, always). The scale channel was measured rather than ignored: `H(scale|context)` = 0.0191 bpw against the already-banked `AdaptiveSuperblockDelta` 0.0673, so the entire conditional-scale prize is bounded at 1.6% of the payload term.

**The controls are what give the negative teeth.** A self control reads exactly 0.0000 with agreement 1.0000 on the same bytes; the marginal reproduces DEC-8.10's banked 3.7525 to 1e-4; and an **adjacent-row control within a single expert also reads 3.7527**, which converts the result from "no correspondence found" to "nothing aligned to find". Mechanism: MXFP4's per-32 e8m0 scale already absorbs per-channel magnitude, leaving residual nibbles near-i.i.d. — the joint-side view of the marginal finding that fp4's log grid is already entropy-matched. **A format that spends its bits well leaves no cross-object redundancy for a dictionary to collect.**

Closed with it, and not to be re-opened on this bank: entropy coding MXFP4 symbols, dictionary coding aligned expert symbols, resident-parent lossless delta coding, adaptive symbol alphabets. **ETC-0B falls too** — a specific parent adds 0.0007 bits over the prototype and selection beats a random parent by −0.0000, so there are no edge weights worth routing around and the DEC-0 residency traces need not be opened for coding-parent work. The routing graph and the compression graph are different objects: routing structure predicts *what will be needed*, not *how to encode it cheaply*.

R4 had already bounded the whole idea before the fetch — routed experts are the smaller half, so zeroing them caps the resident arm at **1.87×**, and the measured 3.753 bpw floor is worth **1.06×** end-to-end. Two things remain formally unmeasured and are deliberately deprioritised: a permutation-*aligned* comparison (an assignment problem over 3072 rows, given a poor prior by the adjacent-row control), and lossy value-space decomposition — a resident shared basis plus a cheap per-expert delta. The second is **approximate expert factorisation, not compression**; it belongs in the approximate lane, must be scored on induced bits/token and route stability rather than reconstruction error, and this lossless result should not be cited for or against it.
