...
jobs:
  wasm32-wasip1:
    ...
    steps:
      ...
      - name: Apply upstream diff (if present)
        run: |
          if [ -f wasm32-unknown-unknown.patch ] && [ -s wasm32-unknown-unknown.patch ]; then
      ...
      - name: Capture cumulative diff
        if: always()
        run: git diff > wasm32-wasip1.patch

      - name: Upload diff artifact
        if: always()
        uses: actions/upload-artifact@v4
        with:
          name: diff-wasm32-wasip1
          path: wasm32-wasip1.patch
          retention-days: 7
...
Note: this file runs standalone (no real `needs:` on the Task 5 workflow — GitHub Actions artifacts don't cross workflow-file boundaries by job name alone). For this standalone experiment, downloading Task 5's artifact by cross-workflow `run-id` is unreliable to script blind — **skip the cross-workflow download for this standalone test** and instead just verify the job builds/tests `wasm32-wasip1` correctly starting from a clean checkout. The real cross-job artifact hand-off only needs to work *within* a single workflow run, which Task 5 already validated and Task 10 exercises for real across the full graph. Simplify Step 1 to drop the download/apply steps for this standalone file:
