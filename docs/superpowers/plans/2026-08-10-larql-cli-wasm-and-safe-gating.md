...
- **Preferred mechanism for the above, added 2026-08-11**: `.github/workflows/clippy-fix.yml` (originally `wasm32-clippy-fix.yml`, renamed 2026-08-11 when generalized — see below) runs `--fix` on a GitHub-hosted runner instead of the local dev machine (`gh workflow run clippy-fix.yml --ref gating/larql-cli-wasm-and-safe -f crate=<crate> -f target=<target, default wasm32v1-none> -f features=<default|no-default-features>`), removing the resource-contention concern that motivated the local exception in the first place. Its `fix` job never commits or pushes — it only uploads a downloadable patch plus a log of whatever `--fix` couldn't auto-resolve. Its `verify-native` job applies that patch on a fresh checkout and runs the real native oracle (`cargo check --all-targets`, `clippy --no-deps -D warnings`, `cargo test`) before anything is judged safe. A green `verify-native` means download the patch and `git apply` + commit locally after reading it; a red one means the patch stays an artifact and the affected items get hand-fixed instead. This makes the immediate-verification contract structural (a `needs:` edge in the workflow) rather than dependent on the operator remembering it.
- **Generalized for reuse, 2026-08-11**: both `crate` and `target` are free-text `workflow_dispatch` inputs, not a hardcoded matrix — a not-yet-existing crate or a not-yet-planned target this campaign later gates on needs zero workflow changes, only a different dispatch argument. `target` defaults to `wasm32v1-none` (this campaign's current scope); `verify-native` itself never takes a `--target` at all, since "the host's native triple" is the correct native-oracle meaning regardless of which target `fix` ran against. Driven by the anticipated need to rebase this branch onto a newer upstream `chrishayuk/larql:main` and to expand gating to additional Rust targets beyond `wasm32v1-none` — this mechanism should not need re-authoring for either. **Also learned the hard way**: a `workflow_dispatch`-only workflow is not dispatchable from a feature branch alone — GitHub only registers `workflow_dispatch` triggers from what's present on the repository's *default* branch (confirmed empirically: absent from `gh api repos/.../actions/workflows` and returns 404 until it exists on `main`, even though the file was already pushed to `gating/larql-cli-wasm-and-safe` — contrast with `pull_request`-triggered workflows, which register from any branch's PR event). `--ref <branch>` then controls which branch's *copy* of the file (and code) a dispatch runs, but only after the workflow is known to `main` at all.
