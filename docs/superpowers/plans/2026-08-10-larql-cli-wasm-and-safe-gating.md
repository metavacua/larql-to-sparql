...
`larql-lql` remains completely untouched — no pattern-2 attribute, no dependency audit. `larql-core --default-features` (pulls in `reqwest`/`rmp-serde`) still fails on all three OSes — not yet investigated, and not a regression (only `--no-default-features` was ever claimed green for this crate).
