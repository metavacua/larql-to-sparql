...
**Status as of this plan revision — real milestone:** `larql-core --no-default-features` compiles cleanly to `wasm32v1-none` on **all three OSes** (ubuntu-latest, macos-14, windows-latest all `success`, confirmed via `gh run view` job-level conclusions on run `31404649309`) — patterns 1-6 all confirmed and fixed via 8 real CI round-trips (commits `fc65bdcb` through `a2df440b`), each one root-caused from the actual raw CI log before any fix was attempted, never guessed. `larql-core --default-features` (which pulls in `reqwest`/`rmp-serde` via its `http`/`msgpack` features) still fails on all three OSes — not yet investigated, expected to need its own pattern(s), likely `native-only-*` given `reqwest` has zero wasm32v1-none support upstream.

Pattern 1 (serde/serde_json/thiserror target-split) is prepared (uncommitted, staged locally) for `larql-vindex-spec`, `larql-models`, `larql-kv`, `larql-vindex`, `larql-inference`, `larql-factory`, `larql-cli` — not yet rolled out or re-verified against patterns 2-6, which weren't known when those edits were staged. `larql-cli` itself is a `[[bin]]` target whose `main.rs` has no `#![no_std]` and won't get one in this pass — it is expected to keep failing on its own implicit std usage even after every dependency/crate-level pattern is cleared; that's the real, final portability boundary this whole exercise exists to find, not a bug to route around.
