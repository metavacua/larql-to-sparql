...
- **`cargo clippy`/`cargo clippy --fix` may be run locally as a narrow, later-approved exception to the rule above** — one invocation at a time, never concurrent with a Workflow/Agent dispatch (a prior session crash was traced to exactly that combination on this 6.3GB dev machine). **`--fix` specifically carries an additional, mandatory verification contract, added 2026-08-11 after a full session was spent tracing native-compile breakage back to unverified `--fix` output**: any `--fix` invocation is incomplete — not "probably fine," not "verify next round" — until an immediate O_native dispatch (`gh workflow run <crate>.yml --ref gating/larql-cli-wasm-and-safe`, real native `cargo test`/`--all-targets`) confirms it, before any other work proceeds on that crate. O_wasm alone is not sufficient: `--fix` (and, just as often, an agent manually narrowing an import or gating an item by hand) can silently delete or mis-scope something that's only load-bearing on the target `--fix` wasn't checking, and that damage produced zero signal for an extended stretch this session specifically because only the wasm32 leg was checked afterward. The default action for any clippy "unused"/"dead code" finding, tool-applied or hand-applied, is `#[cfg(not(target_arch = "wasm32"))]` — never deletion — unless the deletion is justified in place with the specific evidence for it (see `crates/larql-compute/src/phase_timing.rs`'s `enabled()` wasm32 stub removal for what an acceptable justification looks like: it names the exact CI run that proved zero callers on that target).
- **Preferred mechanism for the above, added 2026-08-11, redesigned 2026-08-12**: `.github/workflows/clippy-fix.yml` runs `--fix` on a GitHub-hosted runner instead of the local dev machine, removing the resource-contention concern that motivated the local exception in the first place. No job in it ever commits or pushes — `fix`/`fix-root` only upload a downloadable patch plus a log of whatever `--fix` couldn't auto-resolve; `verify-native`/`verify-native-root` apply that patch on a fresh checkout and run the real native oracle (`cargo check --all-targets`, `clippy --no-deps -D warnings`, `cargo test`) before anything is judged safe. A green `verify-native*` means download the patch and `git apply` + commit locally after reading it; a red one means the patch stays an artifact and the affected items get hand-fixed instead. This makes the immediate-verification contract structural (a `needs:` edge in the workflow) rather than dependent on the operator remembering it. `fix`/`fix-root` also gate `--fix` itself on a plain `cargo clippy` check finding something first, so a crate with nothing to fix costs one clippy invocation, not a full fix+format+diff cycle.
- **Trigger, added 2026-08-11**: `pull_request` (branches: `[main]`), not `workflow_dispatch` — a `workflow_dispatch`-only workflow is not dispatchable from a feature branch alone (GitHub only registers `workflow_dispatch` triggers from what's present on the repository's *default* branch, confirmed empirically: absent from `gh api repos/.../actions/workflows` and 404 until merged to `main`), while `pull_request`-triggered workflows register from any branch's PR event immediately. `workflow_dispatch: {}` (no inputs) is also present as a secondary manual-rerun trigger, matching every other workflow in this repo, usable once this file itself is on `main`.
- **Crate discovery, redesigned 2026-08-12**: no `crate`/`target` typed inputs (a `pull_request` event carries none) and no hardcoded crate list. A `discover` job computes two tiers from `cargo metadata`'s real resolved dependency graph, rooted at a `ROOTS` array (currently `["larql-cli"]`, extended by adding an entry — e.g. a future `larql-server`/`larql-python` gating pass — not by redesigning the workflow): `deps` (every workspace-internal crate transitively reachable from a root, minus the roots themselves and minus `larql-compute-metal`) runs in one flat parallel wave; `roots` runs only after that wave's `verify-native` completes, since a root depends on nearly everything in `deps` and checking it first would conflate the root's own issues with cascading dependency failures. Verified empirically to reproduce the exact 12+1 crate split `larql-cli-gating.yml` already hand-encodes. Three earlier, narrower designs were tried and rejected in the same session: diff-derived discovery (via `git diff` against the PR) silently stops checking a crate that's already red once nothing further touches its src — concretely, `larql-factory` sat red on this campaign's own PR #290, invisible to that design; a hand-copied/runtime-parsed list from `larql-cli-gating.yml`'s own matrix is still a duplicated, driftable list, and only reflects crates someone already decided to gate — useless for finding a crate that needs *first* gating; grepping source for existing `#[cfg(target_arch = "wasm32")]` code has the same circularity. Full narrative: `.superpowers/sdd/2026-08-10-larql-cli-wasm-and-safe-gating/progress.md`.
- **Known gap, not solved as of 2026-08-12**: `fix-root`'s `needs: verify-native` enforces only that the deps tier ran first, not that it's clean — every clippy step in this file uses `continue-on-error: true`, so a dependency with unfixable findings still exits green (empty patch, "nothing to verify" notice), unlike `larql-cli-gating.yml` where a red dependency job actually blocks the root job via GitHub's normal skip-on-fail. `fix-root` therefore still runs even when a dependency is currently wasm32-red. Solving this needs matrix-output aggregation (did ANY deps-tier leg find real findings) that doesn't exist yet.
