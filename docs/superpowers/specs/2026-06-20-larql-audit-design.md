...
- `larql-kv`: Pluggable KV-cache engines — 9 variants: Standard, NoCache, MarkovResidual, MarkovResidualCodec, TurboQuant, UnlimitedContext, BoundaryKv, BoundaryPerLayer, Apollo (retrieval engine)

**External deps**: `tokenizers`, `wasmtime`, `tokio`, `tonic`, `minijinja`, `half`, `memmap2`

**Novel?**: The WalkFfn / sparse FFN retrieval is the central research invention — no OSS equivalent. The KV cache engines (all 9 variants including novel ones: MarkovResidual, MarkovResidualCodec, TurboQuant, BoundaryKv, BoundaryPerLayer) are novel research implementations with no OSS equivalent.

---

### Sub-project E — Query Language (`larql-lql`)
**What it does**: LQL (Lazarus Query Language) parser + executor + REPL. Custom hand-written lexer and recursive descent parser. 30+ statement variants including: USE, DESCRIBE, SELECT, INSERT, DELETE, UPDATE, MERGE, WALK, INFER, COMPILE, EXPLAIN, STATS, DIFF, REBALANCE, TRACE, PIPE, patch lifecycle (BEGIN/SAVE/APPLY/SHOW/REMOVE PATCH), compaction ops (COMPACT MINOR/MAJOR), Show* queries (RELATIONS, LAYERS, FEATURES, ENTITIES, MODELS). Uses `rustyline` for REPL history/completion.
...
### Sub-project H — Knowledge Pipeline (`knowledge/`, package `larql-knowledge`)
...
- `larql-experts`: 20 WASM expert modules compiled to cdylib (arithmetic, dijkstra, graph, markov, sql, statistics, geometry, trig, finance, hash, etc.). **Note**: `larql-experts` is a *nested sub-workspace* at `crates/larql-experts/` with its own `Cargo.toml [workspace]` — it is NOT a member of the root LARQL workspace. It is already structurally independent and buildable standalone.

**External deps**: `wasmtime`, `evalexpr`, `wat`

**Novel?**: The WASM-as-replaceable-kernel pattern is novel. Uses wasmtime (Apache-2.0) correctly. `model-compute` is designed for extraction to a sibling repo (per AGENTS.md); `larql-experts` is already structurally separate.
...
| **KV cache engines** | `larql-kv` — 9 engines: Standard, NoCache, MarkovResidual, MarkovResidualCodec, TurboQuant, UnlimitedContext, BoundaryKv, BoundaryPerLayer, Apollo | None equivalent | — | No OSS project implements markov-residual KV compression, turbo-quant, or boundary-gated KV. | **Keep custom** | These ARE the research. Not replaceable. |
...
The real gap: larql-core's edge model is confidence-weighted, typed, and carries JSON/msgpack IO — it would need a thin wrapper layer over petgraph's `Graph<N, E, Directed>`. The petgraph-replaceable algorithmic core is ~425 LOC (pagerank.rs 101 + shortest_path.rs 148 + traversal.rs 97 + components.rs 79). The remaining ~673 LOC (filter.rs, merge.rs, diff.rs, walk.rs) are LARQL-domain operations that remain in LARQL.
...
This is already identified in `crates/larql-compute/docs/llama-cpp-comparison.md`. The llama.cpp `kernel_flash_attn_ext_vec_reduce` shader (MIT licensed) could be studied or adapted as the basis for D-ATTN-MTG. Writing it from scratch risks repeating the TG-count regression of the 2026-05-01 attempt.
...
|---|---|---|---|
| **P1** | Adopt `petgraph` 0.8.3 as graph backing store in `larql-core`; petgraph covers ALL standard algorithms including PageRank, so keep only LARQL-specific domain ops (merge/diff/walk + typed confidence-edge IO) | 2–3 days | Removes ~425 LOC of petgraph-replaceable algorithm code (pagerank, shortest_path, traversal, components); filter/merge/diff/walk remain in LARQL. |
...
|---|---|---|
| `larql-experts` | **Already structurally separate** — own `Cargo.toml [workspace]` at `crates/larql-experts/` | Buildable standalone today; publish as an independent crate group |
