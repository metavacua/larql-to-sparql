...
**Note on PageRank**: petgraph 0.6+ includes `petgraph::algo::page_rank` (verified via docs.rs). The earlier claim that petgraph lacks PageRank is incorrect — it has it.
...
| **LQL query language parser** | `larql-lql` — hand-written lexer + recursive descent parser | `pest` 2.7 (MIT/Apache), `nom` 7.1 (MIT), `sqlparser-rs` 0.47 (Apache-2.0) | MIT/Apache | All three are mature. `sqlparser-rs` handles SQL-like syntax but adding WALK/DESCRIBE/INFER requires forking its AST. `pest` (PEG grammar files) or `nom` (combinator) would make grammar evolution cleaner without sacrificing control. | **Optional migration** | Hand-written parser works fine for a research project. If LQL grammar grows significantly (e.g. subqueries, complex WALK expressions), migrate to `pest` for a clean formal grammar. `sqlparser-rs` is too opinionated about SQL semantics. |
...
`larql-core` implements BFS/DFS traversal, Dijkstra, A\*, PageRank, connected components, merge — all of which are in `petgraph` 0.8 (MIT/Apache-2.0). Verified on docs.rs 2026: petgraph includes `petgraph::algo::page_rank` (damping factor, iterative, convergence). petgraph is used by rustc and cargo; it is not going anywhere.
...
|---|---|---|---|
| **P1** | Adopt `petgraph` 0.8 as graph backing store in `larql-core`; petgraph covers ALL standard algorithms including PageRank, so keep only LARQL-specific domain ops (merge/diff/walk + typed confidence-edge IO) | 2–3 days | Removes ~500 LOC algorithmic maintenance. petgraph has battle-tested traversal + PageRank. |
...
| **P3** | Evaluate llama.cpp Q4K kernel for parity with LARQL's C kernels | 1 day audit | If llama.cpp's kernel has meaningful improvements (strided block layout, etc.), consider adopting. |
